...
public:
  ...
  void set_mat4x4(const char* name, std::span<const mat4> matrices, bool transpose = false) const
  {
    set_mat4x4(glGetUniformLocation(program, name), matrices, transpose);
  }
  void set_mat4x4(int uniform_location, std::span<const mat4> matrices, bool transpose = false) const
  {
    glUniformMatrix4fv(uniform_location, matrices.size(), transpose, glm::value_ptr(matrices[0]));
  }
